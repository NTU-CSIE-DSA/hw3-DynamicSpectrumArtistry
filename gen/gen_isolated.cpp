...
int main(int argc, char* argv[]) {  // gen_isolated MAX_V MAX_E SCC_count isolated
    registerGen(argc, argv, 1);
    assert(argc == 5);
    int V = atoi(argv[1]);
    int E = atoi(argv[2]);
    int SCC = atoi(argv[3]);
    int isolated = atoi(argv[4]);
    assert(E <= V * (V - 1));
    ...
    while(Edge.size() < E && failed <= 10000){
        int SCC1 = rnd.next(isolated, SCC - 1);
        int SCC2 = rnd.next(isolated, SCC - 1);
        // 只能小->大 or 一樣，不然會少SCC
        if(SCC1 > SCC2){
            int tmp = SCC1;
            SCC1 = SCC2;
